...
var logger = utils.getLogger('Jeadigital-Rebuild_lives-App:[instantiate-chaincode.js]');
...
var instantiateChaincode = async function (peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name) {
	...
	try {
		...
		let results = await channel.sendInstantiateProposal(request, 240000); //instantiate takes much longer
		...
		logger.info('Instantiate proposal response ' + proposalResponses);
	...
	if (!error_message) {
		let message = util.format('Successfully instantiate chaingcode in organization %s to the channel \'%s\'', org_name, channelName);
		logger.info(message);
